...
void insertToEnd(queue_elements** head, queue_elements** tail, threads* newBurst) {
    // Create a new queue element from the burst element
    queue_element* newElement = (queue_element*)malloc(sizeof(queue_element));
    newElement->burst = newBurst;
...
int main(int argc, char* argv[]) 
{
    ...
    while ((read = getline(&line, &len, input_file)) != -1) 
    {
        ...
        printf("bool%d",strcmp(sch_approach,"S") == 0);
        printf("checkpoint 4\n");

        if (strcmp(sch_approach,"S") == 0) 
        {
            printf("%d",pthread_mutex_lock(&queues[0]->lock));
            fflush(stdout);
            
            pthread_mutex_lock(&queues[0]->lock);
            printf("SINGLE QUEUE");
            fflush(stdout);
            insertToEnd(&queues[0],&tails[0],t);
        ...
        else if (strcmp(sch_approach,"M") == 0) 
        {
            ...
            insertToEnd(&queues[q_index],&tails[q_index],t);
